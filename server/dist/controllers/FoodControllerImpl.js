...
class FoodControllerImpl {
    ...
    async deleteFoods(request, response, next) {
        try {
            logger_1.default.info("Deleting foods for user: " + response.locals.userAuth);
            logger_1.default.info("Foods: " + JSON.stringify(request.body));
            const foodDao = new FoodDaoImpl_1.FoodDaoImpl();
            const userAuth = response.locals.userAuth;
            ...
            logger_1.default.info("Successfully deleted foods");
            response.status(200).send("Success");
        }
        catch (error) {
            logger_1.default.error("Error deleting foods for user " +
                response.locals.userAuth + ": " + error);
            response.send(error);
        }
    }
}
exports.FoodControllerImpl = FoodControllerImpl;
